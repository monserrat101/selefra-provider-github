...
func buildHooks(t *testing.T, ctrl *gomock.Controller) github_client.GithubServices {
	...
	mock.EXPECT().ListHooks(gomock.Any(), constants.Testorg, gomock.Any()).AnyTimes().Return([]*github.Hook{&cs}, &github.Response{}, nil)
	...
	mock.EXPECT().ListHookDeliveries(gomock.Any(), constants.Testorg, *cs.ID, gomock.Any()).AnyTimes().Return([]*github.HookDelivery{hd}, &github.Response{}, nil)
